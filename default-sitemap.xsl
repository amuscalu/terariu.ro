...
>
	...
	<a href="/"><span>Prima pagină</span></a>
	...
	<div id="content-head">
		...
		<xsl:choose>
			<xsl:when test="$fileType='RSS'">
				<p>Generată de <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, this is an RSS Sitemap, meant to be consumed by search engines like Google or Bing.</p>
				<p>
			...
			<xsl:otherwise>
				<p>Generată de <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, aceasta este o hartă site XML, menită să fie consumată de motoarele de căutare, cum ar fi Google sau Bing.</p>
				<p>
			...
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Această hartă site conține						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								URL							</xsl:when>
							<xsl:otherwise>
								URL-uri							</xsl:otherwise>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						Acest index de hărți site conține						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								sitemap							</xsl:when>
							<xsl:otherwise>
								hărți site							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				and was generated on 1 februarie 2025 at 7:50			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<a href="/"><span>Prima pagină</span></a>
		...
		<h2 class="empty-sitemap__title">
			Hopa!			<br />
			Nu există articole aici		</h2>
		<div class="empty-sitemap__buttons">
			<a href="/" class="button">Înapoi la prima pagină</a>
					</div>
